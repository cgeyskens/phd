...
# the operation
@dask.delayed
def process_image_manders(filename, name_segments):
    """
    ...
    if filename.endswith(".czi"):
        ...
        # getting the data
        verlap_coeff, overlap_coeff_rot, presynapse_threshold, postsynapse_threshold = calc_synaptic_coloc.manders_coloc(pre_1, post_1)
        # getting the right filename
        img_filename = metadata.image_filename(filename, name_segments) 
        return {
            "img_filename": img_filename,
            "verlap_coeff": verlap_coeff,
...
delayed_results = [process_image_manders(filename, name_segments) for filename in file_list]
